...
__all__ = [
    "WIN_COMBOS", "TicTacToeRoom", "ChessRoom", "CheckersRoom", "chess",
    ...
    def create_bot_room(
        ...
        if game_type == "chess":
            ...
            room = CheckersRoom(
                ...
                host_name=host_name or "Игрок",
                opponent_tg_id=0,
                opponent_name="🤖 Шашечный Бот (ИИ)",
                host_color=host_color,
                is_bot=True
            )
            self.rooms[room_id] = room
            return room
        raise ValueError(f"Режим против бота не поддерживается для {game_type}")
