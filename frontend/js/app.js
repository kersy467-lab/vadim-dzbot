...
async function initApp() {
  ...
  if (roomId && (gameType === "ege_stress_duel" || gameType === "ege_vocabulary_duel")) {
    switchTab("ege");
    if (window.EGE_DUEL?.open) window.EGE_DUEL.open(roomId, gameType);
  } else if (roomId) {
    switchTab("games");
...
function switchTab(tab) {
  ...
  if (btn) {
    ...
    if (activeTab === "games" && tab !== "games") window.GAMES?.cleanup?.();
    if (activeTab === "ege" && tab !== "ege") window.EGE_DUEL?.cleanup?.();
...
function applyPublicArenaMode(me) {
  if (!me || me.has_full_access) return;
  document.querySelectorAll(".tab-btn").forEach(btn => {
    if (btn.dataset.tab !== "ege") btn.classList.add("hidden");
  });
  const navGrid = document.querySelector("nav .max-w-md.grid");
  if (navGrid) { navGrid.classList.remove("grid-cols-5"); navGrid.classList.add("grid-cols-1"); }
  ["date-selector-wrapper", "duty-widget", "daily-fact-widget"].forEach(id => document.getElementById(id)?.classList.add("hidden"));
  activeTab = "schedule";
  switchTab("ege");
}
...
async function loadUserData() {
  ...
  try {
    ...
    if (me && me.full_name) {
      const roleTag = me.role === "admin" ? " • 👑 Админ" : (me.has_full_access ? " • 🎓 Одноклассник" : " • ⚔️ ЕГЭ Арена");
      userBadge.textContent = (me.ege_nickname || me.full_name) + roleTag;
    } else {
      userBadge.textContent = "ЕГЭ Арена";
    }
    applyPublicArenaMode(me);
    return me;
  } catch (e) {
