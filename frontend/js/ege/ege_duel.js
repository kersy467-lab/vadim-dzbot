(function () {
  ...
  function buildQuestion() {
    ...
    const word = random(window.EGE_VOCAB_DATA || ['абонемент']); const indexes = [...word].map((ch, i) => vowels.includes(ch) ? i : -1).filter(i => i >= 0);
    const target = random(indexes); const answer = word[target];
    return { mode: 'vocabulary', word, target, answer, options: [...new Set([answer, ...'аеёиоуыэюя'.split('').sort(() => Math.random() - .5).slice(0, 3)])].sort(() => Math.random() - .5) };
  }
  ...
  function renderBattle() {
    ...
    else { const masked = [...question.word].map((letter,i) => i === question.target ? '_' : letter).join(''); task = `<p class="text-xs text-slate-500">Вставьте пропущенную гласную</p><div class="text-3xl font-black py-3">${esc(masked)}</div><div class="grid grid-cols-4 gap-2">${question.options.map(letter => `<button ${answered ? 'disabled' : ''} onclick="window.EGE.duelAnswer('${letter}' === '${question.answer}')" class="py-3 rounded-2xl bg-slate-100 dark:bg-slate-700 font-black text-lg">${letter}</button>`).join('')}</div>`; }
  ...
  window.EGE_DUEL = { renderDuelTab, initLobby, duelType: value => { type = value; redraw(); }, invite, answer, open, rematch: async () => { room = await window.api.rematchGame(room.room_id); question = null; answered = false; redraw(); } };
})();
