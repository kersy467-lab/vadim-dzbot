...
export function createMarketFinance(container, showToast, onBack) {
  ...
  function renderStocks() {
    ...
    container.querySelector('#market-ipo-open-btn')?.addEventListener('click', async (event) => {
      const rate = Number(container.querySelector('#market-ipo-dividend-rate')?.value); if (rate < 5 || rate > 50) return showToast('Укажите дивиденды от 5% до 50%', 'error');
