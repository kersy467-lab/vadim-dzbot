...
import { marketChange, renderMarketChart } from '../market_chart.js?v=20260925_energy_mechanic_v5_energy_mechanic_v5';
import { renderStateShareMarket } from './state_share_market.js?v=20260925_energy_mechanic_v5_energy_mechanic_v5';
...
const rub = (value) => `${Number(value || 0).toLocaleString('ru-RU', { maximumFractionDigits: 2 })} ₽`;
const pnlRub = (value) => `<span class="${Number(value || 0) >= 0 ? 'text-emerald-600' : 'text-rose-600'} font-mono font-bold">${Number(value || 0) >= 0 ? '+' : ''}${rub(value)}</span>`;
...
export function createMarketFinance(container, showToast, onBack) {
  ...
  async function renderPortfolio() {
    ...
    const instrumentBody = (portfolio.instruments || []).map(row => `<div class="p-3 rounded-xl bg-slate-50 dark:bg-slate-800/50"><div class="flex justify-between"><b>${esc(NAMES[row.instrument_code] || row.instrument_code)}</b><span>${Number(row.quantity).toLocaleString('ru-RU', { maximumFractionDigits: 6 })} ед.</span></div><div class="flex justify-between text-xs"><span>Стоимость: ${rub(row.market_value_rub)} · вложено ${rub(row.cost_value_rub)}</span>${pnlRub(row.unrealized_pnl_rub)}</div><div class="text-[10px] text-slate-500">Закрытые сделки: ${pnlRub(row.realized_pnl_rub)}</div></div>`).join('') || '<div class="text-xs text-slate-400">Инструментов пока нет</div>';
    const paymentsBody = (portfolio.payout_history || []).slice(0, 20).map(row => `<div class="flex justify-between gap-3 p-2 rounded-lg bg-emerald-50 dark:bg-emerald-950/20 text-xs"><span>${esc(row.title)} · ${esc(new Date(row.paid_at).toLocaleString('ru-RU'))}</span><b class="shrink-0">+${money(row.payout_cash)}</b></div>`).join('') || '<div class="text-xs text-slate-400">Выплат пока не было</div>';
    container.innerHTML = shell('Мой портфель', 'Все финансовые активы компании', `<div class="space-y-3"><div class="glass-card rounded-2xl p-4 grid grid-cols-2 gap-2 text-xs"><div><span class="text-slate-500">Баланс</span><b class="block text-base">${money(portfolio.cash)}</b></div><div><span class="text-slate-500">Стоимость активов</span><b class="block text-base">${money(summary.market_value)}</b></div><div>Общий результат<br>${pnl(summary.total_pnl ?? summary.unrealized_pnl)}</div><div>Выплаты<br><b>${money(Number(summary.dividends_earned || 0) + Number(summary.coupons_earned || 0))}</b></div><div class="col-span-2 text-[10px] text-slate-500">Открытая прибыль включает оценку по цене продажи. Для валют и металлов действует спред 1% на сторону, поэтому сразу после покупки результат может быть отрицательным.</div></div><details class="glass-card rounded-2xl p-4" open><summary class="font-black">📈 Акции</summary><div class="space-y-2 pt-3">${stockBody}</div></details><details class="glass-card rounded-2xl p-4"><summary class="font-black">🏛️ Облигации</summary><div class="space-y-2 pt-3">${bondBody}</div></details><details class="glass-card rounded-2xl p-4"><summary class="font-black">🏛️ Акции государства</summary><div class="space-y-2 pt-3">${stateShareBody}</div></details><details class="glass-card rounded-2xl p-4"><summary class="font-black">💱 Валюты и металлы</summary><div class="space-y-2 pt-3">${instrumentBody}</div></details><details class="glass-card rounded-2xl p-4"><summary class="font-black">💸 История выплат</summary><div class="space-y-2 pt-3">${paymentsBody}</div></details></div>`);
    bindBack();
  }
  ...
  function renderBonds() {
    const ownId = Number(store.company?.id || store.company?.company_id || 0);
    const issues = (bonds.bonds || []).map(bond => `<div class="glass-card rounded-xl p-3"><button class="bond-card w-full text-left" data-id="${bond.id}"><div class="flex justify-between"><b>${esc(bond.title)}</b><strong>${money(bond.market_price ?? bond.face_value)}</strong></div><div class="text-xs text-slate-500">Купон ${bond.coupon_rate}% · ${Number(bond.coupon_rate / 2).toLocaleString('ru-RU')}% в день, выплата каждую минуту · ${esc(bondStatusLabel(bond.status))}</div></button>${bond.is_active ? `<button class="bond-primary-buy mt-2 w-full py-1.5 rounded-lg bg-blue-600 text-white text-xs font-bold" data-id="${bond.id}">Купить у государства</button>` : ''}</div>`).join('') || '<div class="text-sm text-slate-500">Активных выпусков пока нет</div>';
    const holdings = (bonds.holdings || []).filter(row => Number(row.quantity || 0) > 0).map(row => `<div class="flex justify-between items-center gap-2 p-2 rounded-lg bg-amber-50 dark:bg-amber-950/20 text-xs"><span>${esc(row.title)} · всего ${Number(row.quantity).toLocaleString('ru-RU')} шт. · доступно ${Number(row.available_quantity).toLocaleString('ru-RU')}</span><button class="bond-list shrink-0 text-blue-600 font-bold" data-id="${row.bond_id}" data-max="${row.available_quantity}">Продать</button></div>`).join('') || '<div class="text-xs text-slate-400">Облигаций пока нет</div>';
  ...
  function renderBondDetail(id) {
    ...
    container.innerHTML = shell(esc(bond.title), 'Государственный выпуск · реальная история вторичного рынка', `<div class="glass-card rounded-2xl p-4 space-y-3"><div class="broker-quote"><div><div class="broker-price">${money(bond.market_price ?? bond.face_value)}</div>${changeHtml(history, ' cash')}</div><div class="broker-spread"><span>Номинал ${money(bond.face_value)}</span><span>Купон ${bond.coupon_rate}% · ${Number(bond.coupon_rate / 2).toLocaleString('ru-RU')}% в день</span></div></div><div class="text-[10px] text-slate-500">Купоны поступают каждую минуту.</div>${chartCard(history, `График ${bond.title}`)}<div class="broker-stat-grid"><div><span>До погашения</span><b>${bond.maturity_days || '—'} дн.</b></div><div><span>Лучшее предложение</span><b>${bond.best_ask ? money(bond.best_ask) : '—'}</b></div></div><details class="broker-book-single"><summary>Предложения вторичного рынка</summary>${asks}</details><button class="bond-buy w-full py-2 rounded-xl bg-blue-600 text-white font-bold">Купить у государства</button></div>`); bindBack(renderBonds);
  ...
  function renderReferenceDetail(code) {
    ...
    container.querySelectorAll('.reference-trade').forEach(btn => btn.addEventListener('click', async () => {
      const side = btn.dataset.side;
      try {
        const portfolio = await NatAPI.getPortfolio();
        const position = (portfolio.instruments || []).find(row => row.instrument_code === code);
        const maxQuantity = side === 'buy'
          ? Math.max(0, Math.floor(((Number(portfolio.cash || 0) - 0.01) / Number(item.buy_rub || 0)) * 1_000_000) / 1_000_000)
          : Number(position?.quantity || 0);
        if (maxQuantity <= 0) return showToast(side === 'buy' ? 'Недостаточно cash для покупки' : 'У вас нет этого инструмента', 'error');
        const suggested = Number(maxQuantity.toFixed(6)).toString();
        const raw = prompt(`Количество (максимум ${suggested}):`, suggested);
        if (raw === null || raw.trim() === '') return;
        const quantity = Number(raw);
        if (!Number.isFinite(quantity) || quantity <= 0) return;
        await NatAPI.tradeReferenceInstrument(code, side, quantity);
        store.setCompany(await NatAPI.getMyCompany());
        await load();
        showToast('Сделка исполнена', 'success');
        renderReferenceDetail(code);
